...
def dictionary_please() -> dict:
    ...
    return {"name": "LUO", "gender": "secret", "city": "Sydney", "age": 7777777}
...
def best_letter_for_pets() -> str:
    ...
    for letter in the_alphabet:
        current_count = len(pet_filter(letter))
...
def make_filler_text_dictionary() -> Dict:
    ...
    for length in range(3, 8):
        wd[length] = [requests.get(url + str(length)).text for _ in range(4)]
    return wd
...
def random_filler_text(number_of_words=200) -> str:
    ...
    words = [
        random.choice(my_dict[random.randint(3, 7)]) for _ in range(number_of_words)
    ]
...
def fast_filler(number_of_words=200) -> str:
    ...
    words = [
        random.choice(my_dict[str(random.randint(3, 7))])
